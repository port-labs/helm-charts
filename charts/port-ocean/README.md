...
| `port.ingestUrl`                                 | Port ingest API base URL (`OCEAN__PORT__INGEST_URL`). Rendered with `tpl` (e.g. `https://ingest.{{ $.Values.global._publicHostedZoneName }}` for regional ingest). Also used for Centralized Ocean lifecycle callbacks ([Ocean #3247](https://github.com/port-labs/ocean/pull/3247)).                                                                                                                                                                                                                                                                                                                            | `https://ingest.getport.io` |
...
Enabled by default in `values.yaml` (`processingMode: dsp`, `lakehouseEnabled: true`). ConfigMaps set `OCEAN__LAKEHOUSE_ENABLED` and `OCEAN__PROCESSING_MODE` after integration config. There is no separate `port.lifecycleUrl`; Ocean uses `port.ingestUrl` / `OCEAN__PORT__INGEST_URL` for lifecycle callbacks ([Ocean #3247](https://github.com/port-labs/ocean/pull/3247)). Port **organization** flags for Centralized Ocean must also be enabled.

To use legacy processing:
