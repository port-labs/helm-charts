...
Port Agent allows to consume and run Port Actions and Workflows in your VPC.
...
**Note:** For HTTP Polling set `STREAMER_NAME=POLLING` and omit `KAFKA_CONSUMER_GROUP_ID`. See the [Streamer Types](#streamer-types) section below for details.
...
| `env.normal.DETAILED_LOGGING`                        | Enables or disables detailed logging. When set to `"true"`, comprehensive request and response information is logged. When set to `"false"`, only summary information is logged. **Note:** Detailed logs may include sensitive information such as authentication tokens and request payloads. | `"true"`                       |
| `env.normal.WEBHOOK_VERIFY_SSL`                      | Enables or disables SSL certificate verification for webhook requests                                                                                                                                                                                                                          | `"true"`                       |
...
- Can run multiple environments with the same Port org ID (using different consumer group IDs)
...
- Simpler setup (HTTP only)

**Considerations:**

- Higher latency compared to Kafka
- Does not support running multiple environments with the same Port org ID (due to shared polling state)
